...
// i've changed the condition in the while loop and set it to currentInvestmentValueEuros <= 100000
    ...
    private static void exercise2() {
        ...
        //While the remaining pounds are higher than 0, you can sell but only if the exchange rate is higher than 1.15
        while (remainingPoundsToSell > 0) { poundToEuroExchangeRate = 1 + (Math.random() * 0.2);
            if (poundToEuroExchangeRate > 1.15) {
                remainingPoundsToSell = remainingPoundsToSell - maxPoundsToSellPerDay;} else {}
            // adding one day for every cicle until the remaining pounds reach 0
        daysToSell++;}

        System.out.println("It took days " + daysToSell + " to exit that cursed economy");
    }
     ...
     * A Example: fori because we know the number of elements
     *
     * Q1: A user inputs their actions into an ATM, they can withdraw, deposit, check balance or exit. What loop should we use?
     * A1: while because we don't want the cycle to be executed once without checking the pin for example
     *
     * Q2: We have a list of 1000 users to send emails to
     * A2: fori because we know the number of elements
     *
     * Q3: We want to print out the first 80 customer names from our bank database
     * A3: while because we don't know the total of the customers of the database
     *
     * Q4: We want to read a file 100 lines at a time, without loading the full file into our program
     * A4: do while because we want to first print 100 lines and then check stopping condition
     */
